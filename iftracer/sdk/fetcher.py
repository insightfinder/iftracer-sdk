...
class Fetcher:
    ...
    def post(self, api: str, body: Dict[str, str]):
        post_url(f"{self._base_url}/v1/iftracer/{api}", self._api_key, body)
...
def fetch_url(url: str, api_key: str):
    response = requests.get(
        ...
        headers={
            ...
            "X-Iftracer-SDK-Version": __version__,
        },
    )
...
def refresh_data(
    ...
    response = fetch_url(f"{base_url}/v1/iftracer/prompts", api_key)
    prompt_registry.load(response)

    response = fetch_url(f"{base_url}/v1/iftracer/pii/tracing-allow-list", api_key)
